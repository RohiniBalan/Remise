...
const brandLogoSchema = new mongoose.Schema({ name: { type: String, required: true, trim: true }, src: { type: String, required: true, trim: true } }, { _id: false });
...
heroContentSchema.statics.getConfig = async function () {
  ...
  if (!config) config = await this.create({ badgeText: 'OFFICIAL F1 COLLECTOR SERIES', title: 'Race Ready.', titleGradient: 'Miniature Speed.', description: 'Experience the thrill of the track with our ultra-realistic, precision-engineered diecast Formula 1 collection.', primaryButtonText: 'Shop Collection', secondaryButtonText: 'View Gallery', carImages: ['/pngcar.png', '/pngcar2.png', '/pngbike2.png'], brands: [{ name: 'Lego City', src: '/logos/city.png' }, { name: 'Technic', src: '/logos/technic.png' }] });
  return config;
};

...
const protect = (req, res, next) => {
  ...
  const rawRole = req.headers['x-user-role'] || 'user';

  // Product service uses "whole_saler" internally,
  // while authentication sends "wholesaler".
  const role = rawRole === 'wholesaler'
    ? 'whole_saler'
    : rawRole;

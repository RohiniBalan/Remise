...
const getShopByCategory = async (req, res) => {
  try {
    ...
    // One aggregation over all products: per category name, get item count
    // and the image of the most-recently-added in-stock product.
    ...
    const data = categories
      .map((cat, i) => {
        ...
        if (count === 0) return null; // no products yet → nothing to show/no image to use

        const style = PALETTE[i % PALETTE.length];
        const isNew = Date.now() - new Date(cat.createdAt).getTime() < TWO_WEEKS_MS;

        return {
          id: cat.name, // matches Product.category for filtering on the category page
          title: cat.name,
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to fetch shop-by-category data',
      error: error.message,
    });
  }
};

module.exports = { getShopByCategory };
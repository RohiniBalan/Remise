...
const CLIENT_SECRET =
  ...
  "YmFjMDMzYjItYjJlOS00NWRkLWFjZDYtYTU1MzU5YzE1ZTJl";
...
router.post("/initiate", async (req, res) => {
  try {
    ...
    if (!redirectUrl)
      return res
        .status(400)
        .json({ success: false, message: "redirectUrl is required" });
    ...
    try {
      const orderRes = await axios.post(
        `${ORDER_SERVICE_URL}/api/orders/internal`,
        orderPayload,
      );
      ...
      console.error(
        "Initiate Error — could not create order via order-service:",
        detail,
      );
      return res
        .status(502)
    ...
    if (paymentMethod === "cod" || paymentMethod === "qr") {
      try {
        ...
        console.error(
          "Initiate Error — post-order-creation step failed (order still created):",
...
router.get("/status/:orderId", async (req, res) => {
  try {
    ...
    if (order && order.paymentStatus !== "SUCCESS") {
      await axios.patch(
        `${ORDER_SERVICE_URL}/api/orders/internal/${orderId}/payment-status`,
        { paymentStatus: "SUCCESS" },
      );
      await deductStock(
        order.items.map((i) => ({
      ...
      if (enrichedOrder.shippingAddress?.phone)
        sendOrderConfirmationSMS(enrichedOrder);

...
import {
  X, MapPin, ShoppingBag, CheckCircle, AlertCircle, RefreshCw,
  Package, Store, Truck, QrCode, Wallet, Search,
} from 'lucide-react';
...
interface SupplierInsufficientLine {
  requestedName: string;
  requestedQuantity: string;
  product: { id: string; title: string; price: number; availableStock: number };
}
interface SupplierStoreResult {
  storeId: string;
  storeName: string;
  storeType?: string;
  distanceKm: number;
  matched: SupplierMatchedLine[];
  insufficientStock: SupplierInsufficientLine[];
  unmatched: string[];
  matchedCount: number;
...
type SupplierStep = 'radius' | 'searching' | 'results' | 'confirming' | 'delivery' | 'payment' | 'placing' | 'success';

const SUPPLIER_RADIUS_OPTIONS = [5, 10, 25, 50, 100];

export default function SupplierCompareModal({
  items, supplierType, store, token, onClose,
}: {
  items: { name: string; quantity: string }[];
  supplierType: 'whole_saler' | 'home_business';
  store: any; token: string; onClose: () => void;
}) {
  const [step, setStep] = useState<SupplierStep>('radius');
  const [radius, setRadius] = useState(25);
  const [customRadius, setCustomRadius] = useState('');
  const [errorMsg, setErrorMsg] = useState('');
  const [results, setResults] = useState<SupplierStoreResult[]>([]);
  const [chosen, setChosen] = useState<SupplierStoreResult | null>(null);
    ...
    navigator.geolocation.getCurrentPosition(
      async pos => {
        try {
          ...
          const storesRes = await smartOrderApi.getNearbyStores(lat, lng, effectiveRadius, supplierType);
          ...
          const matchRes = await smartOrderApi.matchCart(items as any, storeIds);
          const ranked: any[] = matchRes.data.data || [];

          const byId: Record<string, any> = {};
          stores.forEach(s => { byId[s._id] = s; });

          const merged: SupplierStoreResult[] = ranked.map(r => ({
            ...r,
            storeName: byId[r.storeId]?.name || 'Supplier',
            storeType: byId[r.storeId]?.storeType,
          ...
          setErrorMsg(err.response?.data?.message || 'Could not compare suppliers. Please try again.');
          setStep('radius');
        }
      },
      () => {
        setErrorMsg('Location access denied. Enable location to compare nearby suppliers.');
        setStep('radius');
      }
    );
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    setErrorMsg('');
    try {
      const orderItems = chosen.matched.map(m => ({
      ...
      if (paymentMethod === 'qr' && placedOrder?.orderId) {
        await smartOrderApi.confirmQrPayment(placedOrder.orderId, screenshotFile);
      }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg border border-[#BBD5DA] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-[#BBD5DA] bg-[#DFF1F1] flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-base font-bold text-gray-900">Find Best Supplier</h2>
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-4">
          ...
          {(step === 'radius' || step === 'searching') && (
            <div className="space-y-4">
              <p className="text-sm text-gray-600">Choose how far we should search for {supplierType === 'whole_saler' ? 'wholesalers' : 'home businesses'}.</p>
              <div className="flex flex-wrap gap-2">
                {SUPPLIER_RADIUS_OPTIONS.map(r => (
              ...
              <button
                ...
                {step === 'searching'
                  ? <><RefreshCw size={16} className="animate-spin" /> Searching suppliers…</>
                  : <><Search size={16} /> Search within {customRadius || radius} km</>}
              </button>
            </div>
          )}
            ...
            <div className="space-y-3">
              {results.length === 0 && (
                <div className="text-center py-10">
                  <Package size={40} className="text-[#BBD5DA] mx-auto mb-3" />
                  <p className="font-semibold text-gray-700">No nearby suppliers carry these items</p>
                    ...
                    <ul className="mt-2 text-xs text-gray-600 space-y-0.5">
                      {r.matched.map(m => (
                        <li key={m.requestedName} className="flex items-center gap-1.5">
                          <Package size={11} className="text-teal-600 shrink-0" />
            ...
            <div className="space-y-3">
              ...
              <button type="button" onClick={() => { setDeliveryMethod('pickup'); setStep('payment'); }}
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Collect the stock yourself from the supplier.</p>
                </div>
              </button>
              ...
              <button type="button" onClick={() => { setDeliveryMethod('delivery'); setStep('payment'); }}
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">{chosen.storeName} will deliver the stock to your store.</p>
                </div>
              </button>
              ...
              {!paymentMethod && (
                <>
                  ...
                  <button type="button" onClick={() => setPaymentMethod('cod')}
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">Cash / Invoice</p>
                      <p className="text-xs text-gray-400">
                        {deliveryMethod === 'pickup' ? 'Pay at pickup.' : 'Pay on delivery or by invoice.'}
                      </p>
                    </div>
                  </button>
                ...
                <div className="space-y-3">
                  ...
                  {!qrLoading && !storeQr && (
                    <div className="bg-amber-50 border border-amber-200 text-amber-700 rounded-xl p-4 text-sm">
                      This supplier hasn't set up QR payment yet. Please go back and choose Cash/Invoice instead.
                    </div>
                  )}
            ...
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  ...
                  {deliveryMethod === 'pickup' ? 'ready for self pickup' : 'out for delivery'},{' '}
                  paying via {paymentMethod === 'qr' ? 'QR code' : 'cash/invoice'}.
                </p>
              </div>

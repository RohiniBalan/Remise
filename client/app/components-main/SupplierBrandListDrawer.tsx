    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg border border-[#BBD5DA] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-3">
          {titleGroup.brands.map((b, idx) => {
            // Cheapest supplier's stock — suppliers within a group are
            // already sorted ascending by price (see productController groupKey/getGroupedSuppliers).
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-semibold text-gray-900">{b.brand || 'Unbranded'}</p>
                  ...
                  <p className="font-bold text-teal-700 text-lg whitespace-nowrap">₹{b.lowestPrice}</p>
                </div>

                <button onClick={() => onCompareBrand(b)}

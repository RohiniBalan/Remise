...
export default function NavbarHome({ theme, toggleTheme }: NavbarProps) {
  ...
  const visibleNavLinks =
    userData?.role === "user"
      ? [
          ...NAV_LINKS.slice(0, 2),
          { name: "Home Seller", path: "/suppliers" },
          ...NAV_LINKS.slice(2),
        ]
      : NAV_LINKS;
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 right-0 z-[100] ${isScrolled ? "shadow-md" : ""} transition-shadow duration-300`}
      >
          ...
          {verifyBanner && (
            ...
            >
              <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 flex items-center gap-2 text-xs text-amber-800">
                ...
                <button
                  onClick={() => setVerifyBanner(false)}
                  className="ml-2 text-amber-400 hover:text-amber-700 transition shrink-0"
                >
                  <X size={13} />
                </button>
              </div>
        ...
        <div className="bg-[#0FA3B1] text-white text-xs py-2 px-4 text-center font-medium tracking-wide hidden sm:block">
          🎉 Free delivery on prepaid orders above ₹499 &nbsp;|&nbsp; Use code{" "}
          <span className="font-bold underline">REMISE10</span> for 10% off
        </div>

        {/* ── Main Header ────────────────────────────────────────────────── */}
        <header
          className={`${isLight ? "bg-white border-b border-[#BBD5DA]" : "bg-[#0f172a] border-b border-[#2d3748] shadow-[0_0_0_1px_rgba(255,255,255,0.04)]"} transition-colors duration-300`}
        >
          <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 h-16 flex items-center gap-3 lg:gap-5">
            ...
            <Link href="/" className="flex items-center gap-2.5 shrink-0 group">
              ...
              <div className="hidden sm:block">
                <span
                  className={`text-xl font-black tracking-tight ${isLight ? "text-gray-900" : "text-white"}`}
                >
            ...
            >
              <div
                className={`flex items-center w-full rounded-xl border-2 overflow-hidden transition-all ${isLight ? "border-[#BBD5DA] focus-within:border-[#0FA3B1] bg-[#F5F5F5]" : "border-[#334155] focus-within:border-[#0FA3B1] bg-[#111827]"}`}
              >
                <input
                  type="text"
                  value={searchQuery}
                ...
                <button
                  type="submit"
                  className="px-4 py-2.5 bg-[#FF0000] hover:bg-[#e00000] text-white transition shrink-0"
                >
                  <Search size={18} />
                </button>
              </div>
        ...
        {/* ── Category Nav Strip ─────────────────────────────────────────── */}
        <nav
          className={`hidden lg:block border-b ${isLight ? "bg-white border-[#BBD5DA]" : "bg-gray-950 border-white/10"} transition-colors duration-300`}
        >
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex items-center gap-0 h-10">
              ...
              <div className="ml-auto flex items-center gap-4 text-xs">
                <span
                  className={`flex items-center gap-1 ${isLight ? "text-gray-400" : "text-gray-600"}`}
                >
          ...
          {isMobileMenuOpen && (
            ...
            >
              ...
              <div className="px-4 pt-4 pb-2">
                <form
                  onSubmit={handleSearch}
                  className={`flex items-center rounded-xl border overflow-hidden ${isLight ? "border-[#BBD5DA] bg-[#F5F5F5]" : "border-white/10 bg-white/5"}`}
                >
                  <input
                    type="text"
                    value={searchQuery}
                  ...
                  <button
                    type="submit"
                    className="px-4 py-3 bg-[#FF0000] text-white"
                  >
                    <Search size={17} />
                  </button>
                </form>
              ...
              {isLoggedIn && (
                <div
                  className={`mx-4 mb-2 px-4 py-3 rounded-xl ${isLight ? "bg-[#DFF1F1]" : "bg-white/5"}`}
                >
                  <p
                    className={`font-semibold text-sm ${isLight ? "text-gray-900" : "text-white"}`}
                  >
                    {displayName}
                  </p>

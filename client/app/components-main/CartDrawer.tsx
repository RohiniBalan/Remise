      ...
      {isCartOpen && (
        <>
          ...
          >
            {/* Header */}
            <div
              className={`flex items-center justify-between px-5 py-4 border-b shrink-0 ${isLight ? "border-[#BBD5DA]" : "border-white/10"}`}
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2
                    className={`text-sm font-bold ${isLight ? "text-gray-900" : "text-white"}`}
                  >
                    Shopping Cart
                  </h2>
                  <p
            ...
            <div className="flex-1 overflow-y-auto">
              {cart.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center gap-4 px-6 text-center">
                  <div
                    className={`w-20 h-20 rounded-full flex items-center justify-center ${isLight ? "bg-[#DFF1F1]" : "bg-white/5"}`}
                  >
                    <ShoppingBag size={36} className="text-[#BBD5DA]" />
                  </div>
                  <div>
                    <h3
                      className={`font-bold text-lg ${isLight ? "text-gray-800" : "text-white"}`}
                    >
                      Your cart is empty
                    </h3>
                    <p
                ...
                <div className="p-4 space-y-3">
                  {cart.map((item) => (
                    ...
                    >
                      {/* Image */}
                      <div
                        className={`w-20 h-20 rounded-lg shrink-0 overflow-hidden flex items-center justify-center p-1.5 ${isLight ? "bg-white border border-[#BBD5DA]" : "bg-gray-800"}`}
                      >
                        <img
                          src={item.image}
                          alt={item.title}
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div className="flex items-start justify-between gap-2">
                          <p
                            className={`text-xs font-semibold leading-tight line-clamp-2 ${isLight ? "text-gray-800" : "text-white"}`}
                          >
                            {item.title}
                          </p>
                          <button
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          {/* Qty controls */}
                          <div
                            className={`flex items-center rounded-lg border overflow-hidden ${isLight ? "border-[#BBD5DA] bg-white" : "border-white/10 bg-gray-800"}`}
                          >
                            <button
                            ...
                            <span
                              className={`w-7 text-center text-xs font-bold ${isLight ? "text-gray-900" : "text-white"}`}
                            >
                              {item.quantity}
                            </span>
                            <button
                              ...
                              disabled={
                                item.quantity >= (item.totalStock || Infinity)
                              }
            ...
            {cart.length > 0 && (
              <div
                className={`p-5 border-t shrink-0 ${isLight ? "border-[#BBD5DA] bg-white" : "border-white/10 bg-gray-950"}`}
              >
                {/* Free delivery nudge */}
                {subtotal < 499 && (
                  <div
                    className={`flex items-center gap-2 mb-4 px-3 py-2.5 rounded-xl text-xs ${isLight ? "bg-[#DFF1F1] border border-[#BBD5DA] text-teal-700" : "bg-teal-900/30 border border-teal-700/30 text-teal-400"}`}
                  >
                    <Tag size={12} className="shrink-0" />
                    Add ₹{(499 - subtotal).toFixed(0)} more for{" "}
                    <strong className="ml-1">FREE delivery!</strong>
                  </div>
                )}
                ...
                <div className="space-y-2 mb-4">
                  <div
                    className={`flex justify-between text-sm ${isLight ? "text-gray-500" : "text-gray-400"}`}
                  >
                  ...
                  <div
                    className={`flex justify-between items-center pt-3 border-t ${isLight ? "border-[#BBD5DA]" : "border-white/10"}`}
                  >
                    <span

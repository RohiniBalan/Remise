...
export const GATEWAY = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000';
export const INDICTRANS_URL = (process.env.INDICTRANS_URL || 'http://localhost:8400').replace(/\/$/, '');
...
export function parseItems(englishText: string): BulkItem[] {
  const lines = englishText
    ...
    .filter(l => l.length > 1
      && !/^(s\.?no|sl\.?\s*no|no\.|#|item|qty|quantity|total|amount|sr\.?\s*no)\.?\s*$/i.test(l)
      && !/^(grocery|shopping|purchase|(?:to[\s-]?buy)|items?)\s*list\s*:?\s*$/i.test(l));
...
export async function googleOCR(base64: string, mediaType: string, prompt: string): Promise<string> {
  return callGemini([{ inline_data: { mime_type: mediaType, data: base64 } }, { text: prompt }]);
}
...
export async function tryGoogleOCRWithIndicTrans2(base64: string, mediaType: string): Promise<any> {
...
export async function tryClaude(base64: string, mediaType: string): Promise<any> {
...
export async function tryClaudeTextEnrich(productName: string): Promise<{ category: string; description: string; brand: string }> {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) throw new Error('ANTHROPIC_API_KEY not set');
  const { default: Anthropic } = await import('@anthropic-ai/sdk');
  const res = await new Anthropic({ apiKey: key }).messages.create({
...
export async function tryClaudeVoiceProductParse(text: string): Promise<{
  productName: string; category: string; price: number; discountedPrice: number;
  totalStock: number; description: string; brand: string;
}> {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) throw new Error('ANTHROPIC_API_KEY not set');
  const { default: Anthropic } = await import('@anthropic-ai/sdk');
  const res = await new Anthropic({ apiKey: key }).messages.create({
    ...
    messages: [{ role: 'user', content: [{ type: 'text', text: `${VOICE_PRODUCT_PROMPT_INSTRUCTIONS}\n\nTranscribed sentence: "${text}"` }] }],
  });
  const raw = res.content[0].type === 'text' ? res.content[0].text.trim() : '';
...
export async function tryGeminiVoiceProductParse(text: string): Promise<{
  ...
  const raw = await callGeminiText(`${VOICE_PRODUCT_PROMPT_INSTRUCTIONS}\n\nTranscribed sentence: "${text}"`);
  return JSON.parse(cleanJson(raw));
}
...
export async function extractVoiceProductDetails(text: string) {
  ...
  for (const e of engines) {
    ...
    catch (err: any) { console.warn(`[voice-product] ${e.name} failed:`, err.message); }
  }
...
export interface VoiceListItem { name: string; quantity: string; needsClarification: boolean }

export async function tryClaudeVoiceListParse(text: string): Promise<VoiceListItem[]> {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) throw new Error('ANTHROPIC_API_KEY not set');
  const { default: Anthropic } = await import('@anthropic-ai/sdk');
  const res = await new Anthropic({ apiKey: key }).messages.create({
    model: 'claude-haiku-4-5-20251001', max_tokens: 800,
    messages: [{ role: 'user', content: [{ type: 'text', text: `${VOICE_LIST_PROMPT_INSTRUCTIONS}\n\nTranscribed sentence: "${text}"` }] }],
  });
  const raw = res.content[0].type === 'text' ? res.content[0].text.trim() : '';
...
function parseVoiceListJson(cleaned: string): VoiceListItem[] {
  ...
  if (!Array.isArray(parsed)) throw new Error('Voice list parse returned an unexpected format.');
  return parsed
    .map((it: any) => ({
...
export async function tryGeminiVoiceListParse(text: string): Promise<VoiceListItem[]> {
  const raw = await callGeminiText(`${VOICE_LIST_PROMPT_INSTRUCTIONS}\n\nTranscribed sentence: "${text}"`);
  return parseVoiceListJson(cleanJson(raw));
}
...
export async function extractVoiceListItems(text: string) {
  ...
  for (const e of engines) {
    try { return { extracted: await e.fn(), engine: e.name }; }
    catch (err: any) { console.warn(`[voice-list] ${e.name} failed:`, err.message); }
  }
  throw new Error('All voice-parsing engines failed.');
}
...
export type ImageResult = { base64: string; mimeType: string } | { url: string };

export async function generateWithFluxSchnell(productName: string, category: string): Promise<ImageResult | null> {
...
export const CAT_STYLE: Record<string, { bg: string; accent: string; light: string; symbol: string }> = {
...
export async function buildProductCard(productName: string, category: string): Promise<ImageResult> {
...
export async function checkSharedLibrary(productName: string): Promise<ImageResult | null> {
  try {
...
export async function getProductImage(productName: string, category: string): Promise<{ result: ImageResult; aiGenerated: boolean }> {
...
export function imageResultToUrl(result: ImageResult): string {
  return 'base64' in result ? `data:${result.mimeType};base64,${result.base64}` : result.url;
}

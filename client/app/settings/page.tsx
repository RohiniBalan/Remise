...
import {
  ArrowLeft, User, SlidersHorizontal, Shield, Bell,
  Save, Camera, Mail, Phone, Lock, Eye, EyeOff,
  CheckCircle, AlertCircle, Trash2, LogOut,
} from 'lucide-react';
...
function Toast({ msg, ok, onClose }: { msg: string; ok: boolean; onClose: () => void }) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl text-white text-sm font-medium ${ok ? 'bg-green-500' : 'bg-[#FF0000]'}`}>
...
// ── Account Tab ──────────────────────────────────────────────────────────────
function AccountTab({ user, onSave }: { user: UserData; onSave: (u: Partial<UserData>) => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {[
          { label: 'Full Name',     value: fullname,     set: setFullname,     type: 'text',  icon: <User size={15} /> },
          { label: 'Email Address', value: email,        set: setEmail,        type: 'email', icon: <Mail size={15} /> },
          { label: 'Mobile Number', value: mobilenumber, set: setMobilenumber, type: 'tel',   icon: <Phone size={15} /> },
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-[#BBD5DA] p-5 divide-y divide-[#F5F5F5]">
        <Toggle label="Dark Mode"         desc="Switch to dark theme across the app"        value={darkMode}   onChange={setDarkMode} />
        <Toggle label="Newsletter"        desc="Receive weekly deals & product updates"     value={newsletter} onChange={setNewsletter} />
      </div>
      ...
      <button onClick={save}
        ...
        {savedMsg ? <><CheckCircle size={15} /> Saved!</> : <><Save size={15} /> Save Preferences</>}
      </button>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6 max-w-md">
      <div className="bg-[#DFF1F1] border border-[#BBD5DA] rounded-2xl px-5 py-4 flex items-start gap-3">
        ...
        <p className="text-sm text-teal-800">Keep your account secure by using a strong, unique password.</p>
      </div>

      <form onSubmit={handleChange} className="space-y-4">
        {[
          { label: 'Current Password', value: current, set: setCurrent, show: showCurr, toggle: () => setShowCurr(v => !v) },
          { label: 'New Password',     value: newPass, set: setNewPass, show: showNew,  toggle: () => setShowNew(v => !v) },
          { label: 'Confirm Password', value: confirm, set: setConfirm, show: showNew,  toggle: () => setShowNew(v => !v) },
        ...
        {error   && <p className="text-sm text-[#FF0000] flex items-center gap-1.5"><AlertCircle size={14} /> {error}</p>}
        {success && <p className="text-sm text-green-600 flex items-center gap-1.5"><CheckCircle size={14} /> Password updated successfully!</p>}
...
function NotificationsTab() {
  ...
  const Row = ({ k, label, desc }: { k: keyof typeof prefs; label: string; desc: string }) => (
    <div className="flex items-start justify-between py-4 border-b border-[#F5F5F5] last:border-0">
      <div>
        <p className="text-sm font-semibold text-gray-900">{label}</p>
        <p className="text-xs text-gray-500 mt-0.5">{desc}</p>
      ...
      <button onClick={() => toggle(k)}
        ...
        <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 ${prefs[k] ? 'translate-x-5' : ''}`} />
      </button>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <div className="bg-white rounded-2xl border border-[#BBD5DA] p-5">
        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Email Notifications</h3>
        <Row k="orderUpdates" label="Order Updates"   desc="Shipping, delivery, and status changes" />
        <Row k="promotions"   label="Promotions"      desc="Exclusive deals, sales, and coupons" />
        <Row k="emailDigest"  label="Weekly Digest"   desc="A summary of activity and new products" />
      </div>

      <div className="bg-white rounded-2xl border border-[#BBD5DA] p-5">
        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Push & SMS</h3>
        <Row k="nearbyOffers" label="Nearby Offers"  desc="Alerts when stores near you post deals" />
        <Row k="pushEnabled"  label="Push Alerts"    desc="In-browser push notifications" />
        <Row k="smsAlerts"    label="SMS Alerts"     desc="Text message updates for your orders" />
      </div>
...
function SettingsContent() {
  ...
  useEffect(() => {
    if (!localStorage.getItem('token')) router.push('/login?redirect=/settings');
  }, [router]);
    ...
    <div className="min-h-screen bg-[#F5F5F5]">
      ...
      <header className="bg-white border-b border-[#BBD5DA] sticky top-0 z-30 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-gray-900 hidden sm:block ml-2">Settings</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <aside className="w-full lg:w-56 shrink-0">
            <nav className="bg-white rounded-2xl border border-[#BBD5DA] overflow-hidden shadow-sm">
              {TABS.map(tab => (
                ...
                >
                  <span className={activeTab === tab.id ? 'text-[#FF0000]' : 'text-gray-400'}>
                    {tab.icon}
                  </span>
                  {tab.label}
                </button>
          ...
          <section className="flex-1 bg-white rounded-2xl border border-[#BBD5DA] p-6 shadow-sm min-h-[400px]">
            <h2 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2">
              <span className="text-[#FF0000]">{TABS.find(t => t.id === activeTab)?.icon}</span>
              {TABS.find(t => t.id === activeTab)?.label}
            </h2>

            {activeTab === 'account'       && user && <AccountTab user={user} onSave={handleSaveAccount} />}
